...
def gen_aug_mix_batch(device,example_to_aug_example,batch,train_dataset,sent_id_knn_array,knn_mix_ratio,train_size):
    """
    ...
    for sent_id in sent_id_batch:
        ...
        s=int(sentenceid)
        if s in example_to_aug_example:
            regular_batch.append(s)
            augsentid=example_to_aug_example[s]
            sent_id_mix_batch.append(augsentid)
            #select the tensors that have aug version in order to make the batch smaller so their dim align, and then can just index dataset as well
        else:
            regular_batch_noaugpair.append(s)
    ...
    reg_batch = train_dataset[regular_batch]
    reg_batch_noaugpair = train_dataset[regular_batch_noaugpair]
    #mix_batch.to(device)
    #reg_batch.to(device)
    return mix_batch,reg_batch,reg_batch_noaugpair
