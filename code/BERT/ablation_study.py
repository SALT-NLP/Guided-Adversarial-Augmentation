...
def read_data_rule_based_aug(args, tokenizer, labels, pad_token_label_id, mode, 
    ...
    examples = read_examples_from_file("data/conll2003",mode)  #examples = read_examples_from_file_excel("data/conll2003",mode)
    augexamples=[]
    ...
    if mode =='train':
        ...
        random_example_to_example_dict={}

                     
        for augexample in augexamples:
            sent_id = int(augexample.guid.split('-')[1])
            random_example=random.choice(examples)
            #randomsentid=len(examples)
            randomsentid = int(random_example.guid.split('-')[1])
            #random_example.guid="train-"+str(randomsentid)
    
            if sent_id not in example_to_aug_example_dict.keys():
        
        
                example_to_aug_example_dict[sent_id]=randomsentid
                random_example_to_example_dict[randomsentid]=sent_id                   
                #examples.append(random_example)
             
        logger.info("Random dict check length: %s", len(example_to_aug_example_dict.keys()))
        logger.info("Random dict check length: %s", len(random_example_to_example_dict.keys()))
    ...
    if mode is  'train':
        ...
        return dataset,example_to_aug_example_dict, random_example_to_example_dict
    ...
    for epoch in train_iterator:
        ...
        for step, batchorig in enumerate(epoch_iterator):
            ...
            outputs_nomix,loss_nomix = model(inputs_c['input_ids'],target_c,
                                    ...
                                    subtoken_ids=None,
                                    do_intra_mix=False)
            #print('loss_nomix shape: ',loss_nomix.shape)

            loss=torch.cat((loss_mix, loss_nomix), 0)
            #print('totalloss shape: ',totalloss.shape)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (args.evaluate_during_training):
                        ...
                        #results_challenging, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, eval_dataset_challenging, parallel = False, mode="dev", prefix = str(global_step))
                        

                        #if results['f1']>results_challenging['f1']:
                        #if minbool==False:
                        #    minbool=True
                        #else:
                        #    minbool=False
                        #print('minbool ',minbool)
...
def main():
    ...
    if args.do_train:
        ...
        #eval_dataset_challenging = read_data_rule_based_aug_for_dev_set(args, tokenizer, labels, pad_token_label_id, mode = 'dev',
        #                         pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
    ...
    if args.do_predict:
        ...
        test_dataset_challenging = read_data_rule_based_aug(args, tokenizer, labels, pad_token_label_id, mode = 'test', pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
        #if args.test_mode=="regular":
        #    test_dataset = read_data(args, tokenizer, labels, pad_token_label_id, mode = 'test', pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
        #elif args.test_mode=="challenging":
        #    test_dataset= read_data_rule_based_aug(args, tokenizer, labels, pad_token_label_id, mode = 'test', pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
        output_dir = os.path.join(args.output_dir, "best")
