...
parser.add_argument('--train-mode', default='regular', type=str, help='Train with rule based augmentations or regularly')
...
def read_data_rule_based_aug(args, tokenizer, labels, pad_token_label_id, mode,  
    ...
    if mode =='train':       
        augexamples=[]
        if args.augmented_train_percentage!=-1:
        ...
        #Few shot
        file_name="SupportSet.pkl"
        file_path = os.path.join(args.data_dir, file_name)
        open_file = open(file_path, "rb")
        supportset = pickle.load(open_file)
        open_file.close()

        file_name="SupportSetAugmented.pkl"
        file_path = os.path.join(args.data_dir, file_name)
        open_file = open(file_path, "rb")
        supportsetaug = pickle.load(open_file)
        open_file.close()
        ...
        for example in supportset:
            
            supportsentid=len(examples)

            example.guid="train-"+str(supportsentid)
        
            examples.append(example)
        
        for augexample in supportsetaug:
...
def read_data_regular_train_few_shot(args, tokenizer, labels, pad_token_label_id, mode, train_examples = -1, 
              omit_sep_cls_token=False,
              pad_subtoken_with_real_label=False,
              semi = False):
    logger.info("Creating features from dataset file at %s", args.data_dir)
...
def main():
    ...
    if args.do_train:
        ...
        if args.train_mode =="rulebasedaug" :
            train_dataset = read_data_rule_based_aug(args, tokenizer, labels, pad_token_label_id, mode = 'train', pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
        elif args.train_mode =="regular" :
            train_dataset = read_data_regular_train_few_shot(args, tokenizer, labels, pad_token_label_id, mode = 'train', pad_subtoken_with_real_label=args.pad_subtoken_with_real_label)
        ...
        global_step, tr_loss,  test_f1_regular,test_f1_challenging = train(args,train_dataset, eval_dataset, test_dataset_regular,test_dataset_challenging, model, tokenizer, labels, pad_token_label_id)
